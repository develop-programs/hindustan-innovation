...
export function Navbar() {
  ...
  return (
    <header className="relative z-50 flex flex-col w-full">
      <div className="relative z-50 flex bg-linear-to-b from-black/95 via-black-50 to-transparent items-center justify-between px-6 py-4 md:px-10 lg:px-12 w-full">
        ...
        <div className="flex md:hidden items-center">
          <button
            ...
            className="text-zinc-400 hover:text-zinc-100 focus:outline-none bg-zinc-900/50 backdrop-blur-md border border-white/10 p-2 rounded-lg transition-colors"
      ...
      {/* Mobile Navigation */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-zinc-950/95 backdrop-blur-xl border-b border-white/10 px-6 py-6 flex flex-col gap-6 shadow-2xl z-40">
          <nav className="flex flex-col gap-2">
            {navLinks.map((item) =>
              <Link
                ...
                className="px-4 py-3 text-base font-medium text-zinc-400 hover:text-zinc-100 hover:bg-white/5 rounded-xl transition-all duration-300 flex items-center"
              >
                {item}
              </Link>
            )}
